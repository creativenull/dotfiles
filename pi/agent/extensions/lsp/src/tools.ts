...
function formatDiagnostics(result: DiagnosticsResult): string {
  ...
  const hints = diagnostics.filter(
    (d) => d.severity === DiagnosticSeverity.Hint || !d.severity,
  ).length;
  ...
  lines.push(
    `${diagnostics.length} diagnostic${diagnostics.length !== 1 ? "s" : ""}: ${parts.join(", ")}`,
  );
...
function formatCode(diag: Diagnostic): string {
  if (diag.code === undefined || diag.code === null) return "";
  if (diag.source) return ` [${diag.source}(${diag.code})]`;
  return ` [${diag.code}]`;
}
...
export function registerLspTools(pi: ExtensionAPI, manager: LspManager): void {
  pi.registerTool({
    ...
    description:
      "Get compiler/linter diagnostics (errors, warnings, hints) for a file from configured LSP servers. Use after editing files to verify changes did not introduce problems.",
    ...
    parameters: Type.Object({
      filePath: Type.String({
        description:
          "Path to the file to check (absolute or relative to working directory)",
      }),
    }),
